...
public class TourelleAuto extends BaseFx
{ 
	// Body de la tourelle
	...
	private List<FireBallAuto> listBallsAuto;
	// liste des ball à supprimer
	private List<FireBallAuto> listRemoveBalls;
	
	
	
	public TourelleAuto(Vector2f pos)
	{
		...
		this.listBallsAuto = new ArrayList<FireBallAuto>();
		// création de la list à supprimer
		this.listRemoveBalls = new ArrayList<FireBallAuto>();
		
		
	}
		
	public void destroyBall(FireBallAuto ball)
	{
		// suppression
	...
	public void update(Time deltaTime) 
	{
		...
		if(Math.abs(this.angleTourelle - angle) < 0.1f) // la tourelle vise en direction du robo
		{
			...
			if(this.targetContact.getContact() != null && this.targetContact.getContact() == PlayerManager.getSmallRobot().getBody())
			{
				...
				FireBallAuto ball = new FireBallAuto(this,sb,this.bodyTourelle.getPosition().add(diff.mul(4f)),diff.mul(32f));
				this.listBallsAuto.add(ball);
			}
		}
		...
		for(FireBallAuto ball : this.listBallsAuto)
			ball.update(deltaTime);
		
		// appel a la methode update de la liste ball
		this.listBallsAuto.removeAll(this.listRemoveBalls);
	...
	public void draw(RenderTarget render, RenderStates state) 
	{
		...
		for(FireBallAuto b : this.listBallsAuto)
			b.draw(render, state);
